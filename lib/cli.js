...
// list logs
cli.responders.listLogs = () => {
    _logs.list(true, (err, logFileNameArr) => {
        if (!err && logFileNameArr && logFileNameArr.length > 0) {
            cli.verticalSpace();
            logFileNameArr.forEach(logFileName => {
...
// more log info
cli.responders.moreLogInfo = str => {
    // Get the log file name from the provided string
    const arr = str.split('--');
        ...
        _logs.decompress(logFileName, (err, strData) => {
            if (!err && strData) {
                ...
                arr.forEach(jsonString => {
                    const logObject = _utils.parseJSONToObject(jsonString);
