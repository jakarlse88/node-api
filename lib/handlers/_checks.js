...
// Dependencies
const _handlersHelpers = require('./_handlersHelpers');
...
_checks.post = (data, cb) => {
    // Validate inputs
    const protocol = _handlersHelpers.validateProtocol(data.payload.protocol);
    const url = _handlersHelpers.validateURL(data.payload.url);
    const method = _handlersHelpers.validateMethod(data.payload.method);
    const successCodes = _handlersHelpers.validateSuccessCodes(data.payload.successCodes);
    const timeoutSeconds = _handlersHelpers.validateTimeoutSeconds(data.payload.timeoutSeconds);

    if (protocol && url && method && successCodes && timeoutSeconds) {
        // Get the token from the headers
        const token = _handlersHelpers.validateID(data.headers.token);
        ...
        _data.read('tokens', token, (err, tokenData) => {
            if (!err && tokenData) {
                ...
                _data.read('users', userPhone, (err, userData) => {
                    if (!err && userData) {
                        const userChecks = _handlersHelpers.validateUserChecks(userData.checks);
...
_checks.get = (data, cb) => {
    ...
    const checkID = _handlersHelpers.validateCheckID(data.queryStringObject.id);
    ...
    if (checkID) {
        ...
        _data.read('checks', checkID, (err, checkData) => {
            if (!err && checkData) {
                // Get the token from the headers
                const token = _handlersHelpers.getToken(data.headers.token);

                // Verify that the given token is valid
                // and belongs to the user who created the check
                _handlersHelpers.validateToken(token, checkData.userPhone, tokenIsValid => {
                    if (tokenIsValid) {
...
_checks.put = (data, cb) => {
    ...
    const id = _handlersHelpers.validateID(data.payload.id);
...
_checks.delete = (data, cb) => {
    ...
    const id = _handlersHelpers.validateID(data.queryStringObject.id);
                            ...
                            if (!err) {
                                ...
                                _data.read('users', checkData.userPhone, (err, userData) => {
                                    if (!err && userData) {
                                        ...
                                        const userChecks = _handlersHelpers
