...
_users.post = (data, cb) => {
    ...
    const firstName = _handlersHelpers.validateFirstName(data.payload.firstName);

    const lastName = _handlersHelpers.validateLastName(data.payload.lastName);

    const phone = _handlersHelpers.validatePhone(data.payload.phone);

    const password = _handlersHelpers.validatePassword(data.payload.password);

    const tosAgreement = _handlersHelpers.validateTOSAgreement(data.payload.tosAgreement);
...
_users.get = (data, cb) => {
    ...
    const phone = _handlersHelpers.validatePhone(data.queryStringObject.phone);
    ...
    if (phone) {
        ...
        _handlersHelpers.validateToken(token, phone, tokenIsValid => {
            if (tokenIsValid) {
                // Look up the user
                _data.read('users', phone, (err, data) => {
                    if (!err && data) {
