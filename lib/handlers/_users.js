...
const _usersHelpers = require('./helpers/_usersHelpers');
...
_users.post = (data, cb) => {
    ...
    const firstName = _usersHelpers.validateFirstName(data.payload.firstName);

    const lastName = _usersHelpers.validateLastName(data.payload.lastName);

    const phone = _usersHelpers.validatePhone(data.payload.phone);

    const password = _usersHelpers.validatePassword(data.payload.password);

    const tosAgreement = _usersHelpers.validateTOSAgreement(data.payload.tosAgreement);
...
_users.get = (data, cb) => {
    ...
    const phone = _usersHelpers.validatePhone(data.queryStringObject.phone);
    ...
    if (phone) {
        ...
        _tokensHelpers.validateToken(token, phone, tokenIsValid => {
            if (tokenIsValid) {
                // Look up the user
                _data.read('users', phone, (err, data) => {
                    if (!err && data) {
