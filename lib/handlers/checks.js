...
_checks.post = (data, cb) => {
    console.log('_checks.post was called');
    // Validate inputs
                ...
                _data.read('users', userPhone, (err, userData) => {
                    if (!err && userData) {
                        ...
                        if (userChecks.length < config.maxChecks) {
                            // Verify that the given URL has DNS entries 
                            // (and thus that they can resolve)
                            ...
                            dns.resolve(hostName, (err, records) => {
                                if (!err && records) {
                                    ...
                                    cb(400, { 'Error:': 'The hostname of the given URL did not resolve to any DNS entries' });
                                }
                            });
...
_checks.put = (data, cb) => {
    console.log('/api/checks PUT:', data.queryStringObject);
