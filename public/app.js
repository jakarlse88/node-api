...
app.loadChecksListPage = () => {
	// Get phone number from current token. If no token exists, 
	// log user out.
	...
	if (phone) {
		...
		app.client.request(undefined, 'api/users', 'GET', queryStringObject,
			undefined, (statusCode, responsePayload) => {
				if (statusCode == 200) {
					// Determine amount of checks user has
					...
					if (allChecks.length > 0) {
						// Show each created check as a new row in table
						allChecks.forEach(checkID => {
							...
							app.client.request(undefined,
								...
								(statusCode, responsePayload) => {
									if (statusCode == 200) {
										// Make check data into table row
										...
										const state = typeof (responsePayload.state) == 'string' ?
											responsePayload.state : 'unknown';
										td3.innerHTML = state;
										td4.innerHTML = `<a href="/checks/edit?id=${responsePayload.id}">View / Edit / Delete</a>`;
									} else {
										console.log(`Error trying to load check with ID: ${checkID}`);
									}
								});
						});
